...
def main():
    ...
    target_count = min(target_count, 3500)  # 支持最多3500条，留出缓冲
    # 根据实际测试：每次滚动约加载13条数据
    # 计算公式：目标数量 / 13 * 1.2 (留20%缓冲)
    calculated_scrolls = int(target_count / 13 * 1.2)
    max_scroll_attempts = min(calculated_scrolls, 350)  # 最多350次滚动
